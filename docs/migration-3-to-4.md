...
> 站点文档副本：[docs/tutorial/tutorial-extras/migration-3-to-4.md](./tutorial/tutorial-extras/migration-3-to-4.md) · [4.0 说明](./tutorial/tutorial-extras/reactpress-4-0.md)
...
| 场景                                     | 是否需要迁移动作                                   |
| ---------------------------------------- | -------------------------------------------------- |
| 已用 3.x `reactpress dev` + MySQL 的全栈 | 升级全局 CLI；注意 4.0 已精简命令；插件/桌面为增量 |
...
cd your-site
reactpress doctor
reactpress init    # 4.0 全局 CLI：init 即启动（默认 SQLite）
```

全局 CLI 命令：`init` · `doctor` · `logs` · `stop`。`dev` / `theme` / `plugin` / `build` / `start` 等已从全局 CLI 移除。

Monorepo 贡献者：拉取最新代码后 `pnpm install && pnpm build:plugins && pnpm dev`。
...
- `init` 自动安装 featured npm 主题
- 也可在 **Admin → 外观 → 主题** 安装
- 入门：`hello-world`（仓库 local）；生产：`@fecommunity/reactpress-theme-starter`
...
| 全局 CLI 精简                                    | `dev`/`theme`/`plugin` 等不可用 | 改用 Admin 或 Monorepo 脚本           |
| 默认数据库改为 SQLite                            | 不再默认拉起 Docker MySQL       | 需要 MySQL 时见配置文档 / Docker 部署 |
...
- **默认 SQLite**：`.reactpress/reactpress.db`；备份请复制该文件
- **MySQL 全栈模式**：与 3.x 相同，`.env` + `.reactpress/config.json` 结构可用
...
- [ReactPress 4.0](./tutorial/tutorial-extras/reactpress-4-0.md)
- [3.0 平台版](./tutorial/tutorial-extras/reactpress-3-0.md)
