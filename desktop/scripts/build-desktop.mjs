...
async function main() {
  ...
  const existingPackagedAppPath = resolvePackagedAppPath();

  if (!forceRebuild && isElectronPackagingFresh(packagingFingerprint, existingPackagedAppPath)) {
  ...
  const { appPath, manifest } = verifyPackagedApp(resolvePackagedAppPath());
