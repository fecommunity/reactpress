...
export function OptimizeDashboard({ pluginId, pluginActive, config }: PluginSettingsPanelProps) {
  ...
  const jobPercent = job && job.total > 0 ? Math.round((job.processed / job.total) * 100) : undefined;
        ...
        <div className={styles.statsGrid}>
          ...
          <div className={styles.statCard}>
            ...
            <p className={styles.statValue}>
              {report.contentRefs.articles} {t('articles', '文章')} / {report.contentRefs.pages} {t('pages', '页面')}
            </p>
          </div>
          <div className={styles.statCard}>
      ...
      {job ? (
        <div className={styles.progressBlock}>
          <Progress
            percent={jobPercent}
            status={
              job.status === 'failed' ? 'exception' : jobRunning ? 'active' : job.total === 0 ? 'normal' : 'success'
            }
          ...
          <p className={styles.hint}>
            {job.status === 'failed'
              ...
              : jobRunning
                ...
                : t('jobDone', '任务已完成')}
            {' · '}
            {t('processed', '已处理')} {job.processed}/{job.total} · {t('succeeded', '成功')} {job.succeeded} ·{' '}
            {t('failed', '失败')} {job.failed}
            {job.savedBytes > 0 ? ` · ${t('savedBytes', '已节省空间')} ${formatBytes(job.savedBytes)}` : ''}
          </p>
          {jobDone && job.total === 0 ? (
            <p className={styles.emptyJob}>{t('jobEmpty', '没有待优化的图片（可能已全部优化，或原文件已不存在）。')}</p>
          ) : null}
          {job.contentRewrite ? (
            <p className={styles.hint}>
