...
import HeaderSearch from '@/components/reactpress/HeaderSearch';
import LocaleSwitcher from '@/components/reactpress/LocaleSwitcher';
import ThemeSwitch from '@/components/reactpress/ThemeSwitch';
import UserAuth from '@/components/reactpress/UserAuth';
import Link from '@/components/Link';
import Logo from '@/data/logo.svg';
import { getDocumentScrollTop } from '@/src/utils/scroll';
import { getFirstLevelRoute, getIconByName } from '@/src/utils/icons';
    ...
    <header className="relative z-[100] h-16 w-full bg-[var(--bg-box)]">
      ...
      >
        <PageContainer className="relative flex h-16 items-center gap-3 md:gap-4">
          ...
          >
            <span
              className={`block h-0.5 w-[22px] rounded bg-[var(--primary-color)] transition-transform ${
                visible ? 'translate-y-2 rotate-45' : ''
              }`}
            />
            <span
              className={`my-1.5 block h-0.5 w-[22px] rounded bg-[var(--primary-color)] transition-opacity ${
                visible ? 'opacity-0' : ''
              }`}
            />
            <span
