...
/**
 * Monorepo-only CLI — dev, build, server, docker, theme, plugin, etc.
 * Loaded by `reactpress.ts` when `isMonorepoCheckout()` and a legacy subcommand is used.
 * Published npm CLI (`reactpress init` only) does not ship this entry.
 */
...
program
  ...
  .action(async (directory, options) => {
    ...
    if (isMonorepoCheckout(projectRoot)) {
      const result = await initMonorepoProject(projectRoot, {
        force: !!options.force,
        local: !!options.local,
      });
    ...
    await runReactpressCli(args, { cwd: projectRoot });
  });
...
program
  ...
  .action(async (options) => {
    ...
    try {
      const hasOriginFlag =
        options.remoteOrigin !== undefined || options.adminOrigin !== undefined || options.clientOrigin !== undefined;
...
serverCmd
  .command('restart')
  .description(t('cli.server.restart'))
  .action(async () => {
...
serverCmd
  .command('status')
  .description(t('cli.server.status'))
  .action(async () => {
...
dockerCmd
  .command('restart')
  .description(t('cli.docker.restart'))
  .action(async () => {
...
dockerCmd
  .command('status')
  .description(t('cli.docker.status'))
  .action(async () => {
...
nginxCmd
  .command('reload')
  .description(t('cli.nginx.reload'))
  .action(async () => {
    try {
...
themeCmd
  .command('list')
  .description(t('cli.theme.list.description'))
  .action(() => {
