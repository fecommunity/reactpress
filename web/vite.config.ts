...
export default defineConfig(({ mode: viteMode }) => {
  ...
  return {
    base: adminBase,
    staged: {
      // routeTree.gen.ts is generated by @tanstack/router-plugin; vp fmt would rewrite quote style.
      "*": (files) => {
        const filtered = files.filter((file) => !file.endsWith("routeTree.gen.ts"));
        return filtered.length ? [`vp check --fix ${filtered.join(" ")}`] : [];
      },
    },
    ...
    plugins: [
      tanstackRouter({
        ...
        routeFileIgnorePattern: "Login.*|loginHeroSlides|searchDefaults|docs-home",
